...
# Обработка Google Service Account для Railway
...
def setup_google_credentials():
    ...
    if service_account_json:
        # Создаем файл из переменной окружения
        with open(SERVICE_ACCOUNT_FILE, 'w', encoding='utf-8') as f:
            f.write(service_account_json)
        logger.info("Google Service Account создан из переменной окружения")
    elif not os.path.exists(SERVICE_ACCOUNT_FILE):
        logger.error("Google Service Account файл не найден!")
        raise FileNotFoundError("service_account_key.json не найден")

# Вызовите эту функцию в начале
...
MESSAGES = {
    'ru': {
        ...
        'enrollment_form_prompt': "Отлично! Чтобы записаться или получить подробную консультацию, пожалуйста, заполните эту форму:[ https://forms.gle/dMNnnnaRnQv7pagG7 ]. Также Вы можете лично посетить нашу академию по адресу: [Адрес академии из базы знаний]. Мы работаем [График работы из базы знаний]. Будем рады Вас видеть!",
        # Новая фраза для запроса формы при отсутствии информации
        'no_info_form_prompt': "Извините, у меня нет точной информации по вашему вопросу прямо сейчас, или я не нашла такой курс. Чтобы наши менеджеры могли связаться с вами и предоставить подробную информацию, пожалуйста, заполните эту форму: [ https://forms.gle/dMNnnnaRnQv7pagG7 ].",
    ...
    'ky': {
        ...
        'enrollment_form_prompt': "Абдан сонун! Катталуу же кененирээк кеңеш алуу үчүн, сураныч, бул форманы толтуруңуз: [ https://forms.gle/dMNnnnaRnQv7pagG7 ]. Ошондой эле сиз биздин академияга жеке өзүңүз келип кайрылсаңыз болот: [Академиянын дареги базадан]. Биз [Иш убактысы базадан] иштейбиз. Сизди күтөбүз!",
        'no_info_form_prompt': "Кечиресиз, учурда менин сурооңуз боюнча так маалыматым жок, же мен андай курсту тапкан жокмун. Биздин менеджерлер сизге байланышып, толук маалымат бере алышы үчүн, сураныч, бул форманы толтуруңуз: [ https://forms.gle/dMNnnnaRnQv7pagG7 ].",
